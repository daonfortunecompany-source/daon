...
app.get('/api/orders/:orderId/status', async (req, res) => {
  ...
  const deliveredSections = canDeliverHtml ? buildCustomerFacingReportSections(order) : null;
  const payload = {
    orderId: order.id,
...
function stripHealthContentOutsideHealth(section, text) {
  ...
  if (!output) return '';
  if (section === '건강운') return naturalizeRepeatedAdvisories(section, stripRepeatedAdvisoryDisclaimers(section, output));
...
function sanitizeHealthSectionWithoutAppend(text) {
  ...
  output = stripRepeatedAdvisoryDisclaimers('건강운', output);
  output = output
...
function buildCustomerFacingReportSections(order) {
  const source = cloneRuntimeOrder(order?.runtimeReport?.sections || {});
  const promptPayload = buildCustomerFacingPromptPayloadFromOrder(order);
  const ordered = orderReportSections(promptPayload, source);
  const normalized = {};
  for (const [section, value] of Object.entries(ordered || {})) {
    let output = String(value || '');
    output = polishSectionBody(promptPayload, section, output);
...
const GLOBAL_REPORT_DISCLAIMER_TEXT = '※ 이 리포트는 자기이해를 돕는 참고용 해석입니다. 건강 내용은 생활 리듬 점검용으로만 활용하시고, 불편한 증상은 검진이나 전문의 상담으로 확인해 주세요. 투자·법률·계약처럼 중요한 결정도 현실 조건과 전문가 의견을 함께 확인해 주세요.';

const BULLET_HEAVY_SECTIONS = new Set(['재물운', '직업운', '애정운', '자녀운', '건강운', '실천 조언']);
const NORMALIZED_BULLET_LABELS = ['강점', '흐름', '주의', '실천', '핵심', '조언'];

function normalizeInlineBulletTitleBreaks(text, labelPattern) {
  return String(text || '').replace(new RegExp(`(^|\\n)(?!-\\s)([^\\n:]{2,40}?)\\s+-\\s*(${labelPattern}):\\s*`, 'g'), (match, prefix, rawTitle, label) => {
    const title = cleanSectionText(rawTitle).replace(/\s+/g, ' ').trim();
    if (!title) return match;
    if (/[.!?:]$/.test(title)) return match;
    if (/(?:입니다|합니다|습니다|됩니다|있습니다|보입니다|느껴집니다)$/.test(title)) return match;
    return `${prefix}${title}\n\n- ${label}: `;
  });
}

function normalizeUniversalBulletLineBreaks(text, labelPattern) {
  let output = String(text || '');
  for (let pass = 0; pass < 4; pass += 1) {
    const before = output;
    output = normalizeInlineBulletTitleBreaks(output, labelPattern);
    output = output
      .replace(new RegExp(`([^\\n])\\s+-\\s*(${labelPattern}):\\s*`, 'g'), '$1\n- $2: ')
...
function isGeneralAdvisoryDisclaimerParagraph(paragraph) {
...
function stripRepeatedAdvisoryDisclaimers(section, text) {
  const output = cleanSectionText(String(text || ''));
  if (!output) return '';
  const paragraphs = splitParagraphs(output);
  const filtered = paragraphs.filter((paragraph) => {
    const cleaned = cleanSectionText(paragraph);
...
function naturalizeRepeatedAdvisories(section, text) {
  let output = String(text || '');
  if (!output) return '';
  if (section !== '건강운') {
    output = output
      ...
      .replace(/건강운 참고 문구:\s*/g, '')
      .replace(/건강 관련 내용은 참고용 해석이며 검진이 필요할 수 있습니다\.?/g, '건강 관리는 생활 리듬을 먼저 점검해 보시는 편이 좋습니다.')
      .replace(/중요한 결정은 현실 조건과 전문가 상담을 함께 고려하세요\.?/g, '중요한 선택일수록 조건과 책임 범위를 끝까지 확인해 보세요.')
  ...
  output = output
    .replace(/[ \t]{2,}/g, ' ')
    .replace(/-\s*(강점|흐름|주의|실천|조언|핵심):\s*(?=(?:-\s*(강점|흐름|주의|실천|조언|핵심):|$))/g, '')
    .replace(/^\s*-\s*$/gm, '')
    .replace(/\n{3,}/g, '\n\n');
  return cleanSectionText(output);
}

function hasMoneyTimingConcern(promptPayload) {
  const concern = String(promptPayload?.basicInfo?.concern || promptPayload?.applicant?.concern || '').trim();
  return /(돈|재물|수입|소득|연봉|월급|부업|자산|투자|벌 수|벌수)/.test(concern);
}

function buildMoneyTimingHint(promptPayload, section) {
  const name = formatHonorificName(promptPayload?.basicInfo?.name || promptPayload?.applicant?.name || '') || '고객님';
  const { year } = getBaselineYearMonth(promptPayload);
  const currentRange = buildCurrentDaeunPeriodLabel(promptPayload) || '현재 대운';
  if (section === '재물운') {
    return `시기를 조금 더 선명하게 보면 ${year}년 하반기부터는 ${name}의 돈 흐름이 단번에 커진다기보다, 본업 전문성·계약 조건·부업 연결·연봉 협상처럼 현실적인 조건이 실제 수입으로 이어지는지에 따라 체감 차이가 커질 수 있습니다. 특히 ${currentRange}의 후반부로 갈수록 큰 한 번보다 꾸준히 이어지는 수입 구조를 붙일 때 재물운의 안정감이 더 분명해질 수 있습니다.`;
  }
  return `“언제 돈을 벌 수 있을까요?”라는 질문에는 한 시점을 단정하기보다, ${year}년 하반기부터 ${currentRange}의 후반부로 갈수록 ${name}의 전문성, 계약 조건, 부업, 연봉 협상 같은 현실 요소가 수입으로 연결되는지 보시는 편이 더 정확합니다. 특히 역할이 선명해지고 조건을 다시 잡는 순간에 돈 버는 흐름이 조금 더 또렷하게 체감될 수 있습니다.`;
}

function ensureMoneyTimingGuidance(promptPayload, section, text) {
  if (!['재물운', '고민에 대한 조언'].includes(section)) return cleanSectionText(text);
  if (!hasMoneyTimingConcern(promptPayload)) return cleanSectionText(text);
  let output = cleanSectionText(String(text || ''));
  if (!output) return output;
  const hasTimingCue = /(2026년|상반기|하반기|후반부|연봉 협상|부업|계약 조건|수입 구조)/.test(output);
  if (!hasTimingCue) output = `${output}\n\n${buildMoneyTimingHint(promptPayload, section)}`;
  return cleanSectionText(output);
}
