...
function removeDuplicateHealthIntroAndDisclaimer(text) {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (paragraph === '건강운에서는 생활 리듬, 피로 누적 패턴, 회복 습관과 컨디션 관리 포인트를 설명합니다.') {
      if (!introAdded) {
    ...
    if (paragraph === disclaimer || /건강운은 의학적 진단이 아니라 생활 습관을 점검하기 위한 참고용 해석/.test(paragraph)) {
  ...
  if (!disclaimerAdded) rebuilt.push(disclaimer);
  return cleanSectionText(rebuilt.join('\n\n'));
}

function cleanupConcernTail(text) {
  return cleanSectionText(String(text || '')
    .replace(/고민 반영 보강 해석\s*현재 입력된 고민이 구체적이지 않기 때문에,[\s\S]*?판단 피로를 줄이는 데 도움이 됩니다\.?/g, '')
    .replace(/고민 반영 보강 해석\s+현재 입력된 고민이/g, '고민 반영 보강 해석\n\n현재 입력된 고민이'));
}
...
function ensureSectionQuality(promptPayload, section, text) {
  ...
  const needsParagraphs = countMeaningfulParagraphs(output) < desiredParagraphs;
  ...
  if ((needsParagraphs || missingContent) && supplement) {
    output = `${output}
...
function buildMissingSectionFallback(promptPayload, section) {
  ...
  const map = {
    ...
    '운성': buildUnseongSupplement(promptPayload),
    '신살·귀인': buildShinsalSupplement(promptPayload),
    '십성': `십성에서는 ${name}이 관계와 일에서 어떤 역할을 편하게 수행하는지 먼저 정리합니다. 현재 자료를 기준으로 보면 기준을 세우고 책임을 나누는 방식이 중요한 해석 포인트가 될 수 있습니다.`,
    '재물운': `재물운에서는 ${name}의 수입 흐름과 지출 관리 패턴을 현실적인 언어로 정리합니다. 현재 자료를 기준으로 보면 수입을 넓히는 시도만큼이나 새는 지출을 먼저 줄이는 기준을 세우는 일이 중요할 수 있습니다.`,
    '직업운': `직업운에서는 ${name}이 어떤 업무 환경에서 힘을 덜 쓰고도 강점을 살리기 쉬운지부터 살펴봅니다. 현재 흐름에서는 역할 범위와 협업 구조를 분명히 하는 환경이 더 안정적으로 맞을 수 있습니다.`,
    '애정운': `애정운에서는 ${name}이 관계에서 중요하게 보는 기준과 정서적 안정감을 주는 패턴을 차분히 정리합니다. 현재 흐름에서는 감정보다 생활 리듬과 약속의 일관성이 더 중요한 판단 기준이 될 수 있습니다.`,
    '자녀운': `자녀운에서는 실제 자녀의 유무를 단정하지 않고, ${name}이 누군가를 돌보고 성장시키는 태도를 중심으로 살펴봅니다. 현재 자료를 기준으로 보면 책임감은 강하지만 통제보다 대화가 더 중요한 흐름으로 읽을 수 있습니다.`,
    '건강운': `건강운에서는 ${name}의 체력 소모 패턴과 회복 리듬, 생활 습관 관리 포인트를 점검합니다. 현재 흐름에서는 과로가 쌓이기 전에 수면과 식사, 운동 리듬을 먼저 고정하는 태도가 중요할 수 있습니다.`,
    '실천 조언': `실천 조언에서는 ${name}이 지금 바로 적용할 수 있는 행동 기준과 생활 조정 포인트를 정리합니다. 한 번에 많은 계획을 세우기보다 유지 가능한 작은 실천을 먼저 고정하는 편이 더 현실적일 수 있습니다.`,
    '주의할 점': `주의할 점에서는 ${name}이 무리하거나 놓치기 쉬운 패턴을 미리 점검합니다. 현재 흐름에서는 조급한 결정과 누적 피로 속 판단이 가장 먼저 관리해야 할 지점이 될 수 있습니다.`,
    '고민에 대한 조언': buildConcernSupplement(promptPayload)
  };
  return map[section] || `${name}의 ${section} 흐름을 현재 자료 기준으로 차분히 정리합니다. 단정적인 예언보다 실제 생활에 적용할 수 있는 기준과 조절 포인트를 중심으로 읽어보시는 편이 좋습니다.`;
}
...
function buildSectionSpecificPromptInstructions(sections, promptPayload = {}) {
  ...
  const { year, month } = getBaselineYearMonth(promptPayload);
  if (sections.includes('대운')) {
    lines.push('대운은 반드시 하나의 나이대 기준으로만 작성하세요. 24세~33세, 34세~43세, 44세~53세처럼 통일하고, 각 구간마다 해당 나이대, 주요 흐름, 직업/돈/관계 중 최소 2가지 이상, 주의할 점, 실천 조언을 포함하세요. 마지막에는 반드시 대운 3줄 요약을 넣으세요. 20대 후반~30대 초반 같은 다른 기준을 섞지 마세요.');
  }
  if (sections.includes('세운')) {
    lines.push(`${year}년 세운이라는 점을 첫 문장에 분명히 밝히고, 일/커리어, 돈/재물, 관계/협업, 건강/컨디션, 올해의 선택 기준을 나누어 작성하세요.`);
  }
  ...
  if (sections.includes('건강운')) {
    lines.push('건강운 도입문은 하나만 사용하고, 건강운 참고 문구는 본문 마지막에 한 번만 넣으세요. 동일하거나 유사한 도입문과 문구를 두 번 반복하지 마세요.');
  }
  if (sections.includes('고민에 대한 조언')) {
    lines.push('고민 반영 보강 해석 문단이 이미 본문에서 설명된 내용과 중복되면 반복하지 마세요. 특히 현재 입력된 고민이 구체적이지 않기 때문에로 시작하는 보강 문단이 뒤에 또 붙지 않도록 하세요.');
  }
  return lines.join(' ');
}
  ...
  if (batchResult?.splitRecommended && batch.sections.length > 1) {
    ...
    for (const section of regenTargets) {
      ...
      try {
        ...
        if (CONFIG.allowLocalFallback) {
          Object.assign(merged, repairSectionsWithFallback(promptPayload, { ...merged, ...(error?.partialSections || {}) }, [section], error?.message || 'single_section_failed'));
          continue;
        }
      ...
      if (!singleResult?.ok) {
        if (CONFIG.allowLocalFallback) {
          Object.assign(merged, repairSectionsWithFallback(promptPayload, { ...merged, ...(singleResult?.sections || {}) }, [section], 'single_section_validation_failed'));
          continue;
        }
...
async function generateAiSections(promptPayload, order = null) {
  ...
  if (finalValidation.ok) console.log('[KIE AI FINAL] validation success', JSON.stringify({ requiredSections, failedSections: finalValidation.failedSections || [] }));
  if (!finalValidation.ok) {
    ...
    const softErrorPatterns = [
      /분량 부족/,
      /total_section_length_too_short/,
      /건강운 참고 문구 중복/,
      /대운 구조 기준 불일치/
    ];
    const hardErrors = (finalValidation.errors || []).filter((error) => !softErrorPatterns.some((pattern) => pattern.test(String(error || ''))));
    const hasAllRequiredSections = requiredSections.every((section) => String(processedFinalSections?.[section] || '').trim());
    if (hasAllRequiredSections && hardErrors.length === 0) {
      console.log('[KIE AI FINAL] soft validation accepted', JSON.stringify({ failedSections, softErrors: finalValidation.errors || [] }));
      return processedFinalSections;
    }
    if (CONFIG.allowLocalFallback && failedSections.length) {
      const repairedSections = repairSectionsWithFallback(promptPayload, processedFinalSections, failedSections, (finalValidation.errors || []).join(' | ') || 'final_validation_failed');
      const repairedValidation = validateSectionMap(repairedSections, promptPayload, {
        rawLength: totalLength,
        detectedFormat: 'merged_sections_repaired',
        sourcePath: 'batch_merge_repair'
      }, {
        requiredSections,
        enforceTotalLength: false,
        requireRawLength: false
      });
      console.log('[KIE AI FINAL] repaired validation result', JSON.stringify(repairedValidation));
      const repairedHardErrors = (repairedValidation.errors || []).filter((error) => !softErrorPatterns.some((pattern) => pattern.test(String(error || ''))));
      const repairedHasAllRequiredSections = requiredSections.every((section) => String(repairedSections?.[section] || '').trim());
      if (repairedHasAllRequiredSections && repairedHardErrors.length === 0) return repairedSections;
    }
    ...
    if (!CONFIG.allowLocalFallback) {
      ...
      fatal.partialSections = { ...processedFinalSections };
      throw fatal;
    }
    return repairSectionsWithFallback(promptPayload, processedFinalSections, failedSections.length ? failedSections : requiredSections, 'final_fallback_completion');
  }
  return processedFinalSections;
}
