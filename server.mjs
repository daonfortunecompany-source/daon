...
function getFinalChecklistSectionTitle() {
  return '최종 실천 체크리스트';
}

function orderReportSections(promptPayload, sections, shouldAppendChecklist = false) {
  const ordered = {};
  ...
  const checklistTitle = getFinalChecklistSectionTitle(promptPayload);
  const shouldIncludeChecklist = shouldAppendChecklist || (typeof source[checklistTitle] === 'string' && source[checklistTitle].trim());
  if (shouldIncludeChecklist) {
    ordered[checklistTitle] = cleanSectionText(source[checklistTitle] || buildFinalChecklistSection(promptPayload));
  }
...
function postProcessReportSections(promptPayload, sections) {
  ...
  const shouldAppendChecklist = normalizedSourceKeys.filter((key) => requiredSections.includes(key)).length >= Math.max(10, requiredSections.length - 1);
  ...
  return orderReportSections(promptPayload, output, shouldAppendChecklist);
}
...
function validateAiSectionsLegacy(sections, promptPayload, meta = {}, options = {}) {
  ...
  const checklistTitle = getFinalChecklistSectionTitle(promptPayload);
  if (!sections?.[checklistTitle]) {
    errors.push(`${checklistTitle} 누락`);
  }
