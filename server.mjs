...
function buildConcernSpecificInstruction(concern, hasCompatibility, birthTimeUnknown = false) {
  const lines = [
    ...
    '각 섹션은 의미 설명, 핵심 해석, 실제 생활 예시, 주의할 점, 실천 조언 2~3개를 자연스럽게 담고, 말미에는 필요할 때만 짧은 정리나 실천 포인트로 마무리하세요.',
  ...
  if (!String(concern || '').trim()) {
    lines.push('사용자 고민이 비어 있거나 구체적이지 않다면 질문 응답 형식 대신 현재 흐름 전반에 대한 조언처럼 자연스럽게 작성하세요.');
  }
  return lines.join(' ');
}
